(ns automationanywhere.main
  "Kotodama WASM entrypoint for the Automationanywhere clean-room actor (L4) — Clojure port.
  ...
  facts (`automationanywhere.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "automationanywhere")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"automationanywhere.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "automationanywhere-compat" :tier tier :entities entities} 200])
...
(defn start! [] :automationanywhere-compat/ready)
